...
<%--
	1. 자바스크립트에서 태그를 제어하는 메소드 (읽기/쓰기)
		...
		=================================
		= 태그에 id 속성이 있는 경우
			document.getElementById("아이디") // Element : 태그
		= 태그에 name 속성이 있는 경우
...
function result(){
	// 서버에 전송하지 않고 브라우저에서 자체 처리 : 자바스크립트
	// 사용자가 입력한 값을 받는다
	...
	let avg=total/3
	document.getElementById("avg").value=Math.round(total/3,2)
			...
			<table class="table">
				...
				<tr>
					<td colspan="2" class="text-center">
						<input type="button" value="성적 처리" class="btn-sm btn-danger"
						onclick="result()">
					</td>
				</tr>
			</table>
