...
<!--
	...
	a.jsp => 전송한 값을 공유
		...
		=> d.jsp
		========= session 이용
		          ======= static (공유할 때 필요 시마다 어떤 JSP든 상관 없이 사용 가능)
		          session은 사용자 브라우저마다 1개 생성 (메모리 공간) => 데이터는 여러개 저장 가능

	============================
	1. 요청값 전송
		ex) 검색 => 사용자의 검색어를 받는다 (검색창)
			로그인 => id, pwd => ID 저장, 자동 로그인
			페이지
	2. 요청값을 받아서 데이터베이스 연동 : SQL => DAO
	3. HTML 실행 화면 : 자바 => for, if
	============================
	자바 : Model : DAO, VO, Service
	HTML : View : 브라우저 이벤트 (자바스크립트)
	=> MV 패턴
		=> 기능별 메소드 => JSP 호출
	...
	내장 객체
		...
		session : 서버에 데이터를 저장하여 모든 JSP에서 공유 목적
...
-->
<%
	// diary_list.jsp?year=&month=&day= => (_jspService(){소스 코딩})
				...
				<tbody>
					...
					%>
							<tr>
								...
								<td width=30% class="text-center"><%=dvo.getDbday() %></td>
								<td width=20% class="text-center">
									<span class="btn btn-xs btn-warning checks" data-no="<%=dvo.getNo() %>">일정 확인</span>
