...
public class MovieManager {
	...
	public List<MovieVO> movieListData(int type)
	{
		...
		try
		{
			...
			// 필요한 데이터를 읽어 온다 => 파서 (HTML(Jsoup)/XML(DocumentBuilder)/JSON(SimpleJSON))
			// [{},{},...] = [] => JSONArray => List
			// {} => JSONObject => VO
			...
			// 1. HTML => 크롤링
			// 2. XML => 설정 파일 (MyBatis, Spring)
			// 3. JSON => 자바와 자바스크립트 호환 => Ajax / Vue / React
			// 4. 공공 데이터 포털, 네이버 데이터 센터, 서울시 데이터 센터
